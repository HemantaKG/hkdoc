...
````
If you want ot keep the firewall service running =>
...
````
./configure --with-devel-headers --prefix=/extra/openmpi3_slurm --with-pmi=/usr --with-pmi-libdir=/usr --with-slurm --with-sge
make -j 4 all
make install
````
## Update node state
